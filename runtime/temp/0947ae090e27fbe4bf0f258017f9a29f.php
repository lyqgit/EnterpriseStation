<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:62:"F:\wamp\www\wlcg/application/admin\view\index\admin_power.html";i:1499693476;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="javascript:;" onclick="admin_permission_add('添加权限节点','power_add.html','1000','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加权限节点</a></span> <span class="r">共有数据：<strong><?php echo count($power); ?></strong> 条</span> </div>
		...
		<tbody>
			<?php if(is_array($power) || $power instanceof \think\Collection || $power instanceof \think\Paginator): $i = 0; $__LIST__ = $power;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
				<tr class="text-c">
					...
					<td><a title="编辑" href="javascript:;" onclick="admin_permission_edit('权限编辑','<?php echo url('admin/admin/power_edit_show',['authority_id'=>$vo['authority_id']]); ?>','<?php echo $vo['authority_id']; ?>','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_permission_del(this,'<?php echo $vo['authority_id']; ?>','<?php echo $vo['authority_group']; ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
...
/*管理员-权限-删除*/
function admin_permission_del(obj,id,id2){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php echo url("admin/admin/power_del"); ?>',
			data: {'authority_id':id,'authority_group':id2},
			dataType: 'json',
